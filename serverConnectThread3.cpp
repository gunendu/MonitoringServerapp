...
serverConnectThread3::serverConnectThread3(int nThreadIndex) {
...
serverConnectThread3::~serverConnectThread3() {
    //serverObject.clear();
}
...
*/
RET_CODE serverConnectThread3::waitsocket(int socket_fd, LIBSSH2_SESSION *session) {
...
//
//}
RET_CODE serverConnectThread3::Process(void* pcThreadParam) {
   ...
   // char *lcPreferredAuthMethod[20];
    ...
    while (1) {
        ...
        for (int i=(singleInstance->serverObject.size()-singleInstance->serverObject.size()/3);i<singleInstance->serverObject.size();i++) {
	...
	//	tsi.bIsDirty = true;
	    ...
	    cout <<"hostAdress3 ::: " << tsi.ipAdress << endl;
