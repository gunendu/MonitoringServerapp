...
int ConfigParser_t::readFile(const string &fname) {
    ...
    if (fpvec.back() == NULL) {
        ...
        //std::cout << "returned from here" << std::endl;
        return -1; /*error*/
    }
    ...
    while (fpvec.size() > 0) {
        ...
        if (i < buf.length() && buf[i] == '[') {
            ...
            //std::cout << "section name " << ent.section << std::endl;
        ...
        if (ent.name.length() >= 9
            ...
            fpvec.push_back(fopen(ent.name.c_str() + 9, "rb"));
            if (fpvec.back() == NULL) {
...
int ConfigParser_t::getValue(string section, string name, string *value) const {
    ...
    while (i < entry.size()) {
        ...
        //std::cout << entry[i].section << ", " << entry[i].name << std::endl;
 ...
 */
int ConfigParser_t::getValue(string section, string name, vector<string> *value) const {
