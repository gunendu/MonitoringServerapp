...
serverConnectThread2::serverConnectThread2(int nThreadIndex) {
    //singleInstance = CControllerThread::getInstance();
    m_threadInfo.nThreadIndex = nThreadIndex;

}

serverConnectThread2::~serverConnectThread2() {
    //serverObject.clear();
}
...
/*RET_CODE serverConnectThread::ParseServerConfigFile(const char* pcFileName, tagServerInfo& ServerInfo,StartLineProcessConnector *lpconn) {
    ...
    for (int i = 1; i < noCons + 1; i++) {
        ...
        if (0 == lCConfigParser_t.getValue(section, "startPath", &lstrBuffer)) {
            

        }
        startPath = lstrBuffer;
        lstrBuffer.clear();
        std::cout<<"startPath"<<startPath<<endl;*/
...
*/
RET_CODE serverConnectThread2::waitsocket(int socket_fd, LIBSSH2_SESSION *session) {
...
//
//}
RET_CODE serverConnectThread2::Process(void* pcThreadParam) {
   ...
   // char *lcPreferredAuthMethod[20];
    ...
    while (1) {
        ...
        for (int i=(singleInstance->serverObject.size()/3);i<(singleInstance->serverObject.size()-singleInstance->serverObject.size()/3);i++) 
	  {
	    ...
	    cout <<"hostAdress2 ::: " << tsi.ipAdress <<endl;
 ...
 */
	...
	singleInstance	 g_cSystemLogger.LogMessage("(%s:%s(:%d))::Inside ColoBridge \n", LOG_LOCATION);
		...
		std::cout<<"LineType"<<tsi.lineType<<endl;
            ...
            CcopyStructure::copyData(tsi, tsi.noOfProcess, tsi.noOfSpace, globalCounter);
